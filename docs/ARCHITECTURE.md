...
| Scope → SQL predicate for raw SQL / Dapper | ✅ built (009 P1–P5 merged) | `Aperture.SharedKernel/Data/ScopedConnection.cs`, `/Data/RowLevelSecurity/ScopeRlsPolicy.cs`, `/Data/ScopeSessionContext.cs`, `Authorization/ScopeSql.cs`; gate: `RawSqlIsScopedTests.cs`, `scripts/measure.sh rawsql` | measured 2026-09-03: raw SQL reaches the DB only through `ScopedConnection` as the least-privilege `aperture_reader` role; Postgres RLS re-asserts tenant + scope below the SQL string (fail-closed on unset context). The old `/**scope**/` placeholder mechanism was removed as fail-open; PR #24 was superseded by the RLS design and P3–P5 merged. Differential test proves EF `WhereInScope`, the P2 fragment, and the RLS policy select the identical id set. `measure.sh rawsql`: 0 production call sites. Wired into API DI; consumed by the three Sales grids (002). ~~Measured 2026-09-24 (231c593): the three raw-SQL detectors disagree~~ — *corrected 2026-09-25 (011-P7):* `RawSqlIsScopedTests`, `measure.sh rawsql` and GATE 2 now all read one pattern and one exemption list from `scripts/rawsql-rules.txt` (covering `ExecuteSql*`/`FromSql*`/`SqlQuery*`); a parity test proves grep and .NET agree on every match in `src/`. `Development/DemoSeed.cs:209,212` is exempt in all three by exact path. |
| Dapper (as a dependency) | ✅ present | `Directory.Packages.props`, `Aperture.SharedKernel/Data` | measured 2026-09-03: Dapper 2.1.66 pinned centrally, referenced by exactly one project (`Aperture.SharedKernel/Data`, the sanctioned wrapper); an architecture test enforces the single reference. §4's "Dapper owns list and report queries" is now true of the read path, and its call sites are the three Sales grid reads (`AccountService`/`ContactService`/`DealService`), all through `ScopedConnection` (re-measured 2026-09-24) |
| Sales: accounts, contacts, deals | ✅ built | `src/Modules/Sales` — 4 tables in the `sales` schema (`accounts`, `contacts`, `deals`, `deal_lines`); 13 order-agnostic routes on `accounts.*`/`contacts.*`/`deals.*` | measured 2026-09-25 (dc43301, 011 done): `tests` counts Sales 104 test methods, Api 104 (`[Fact]`/`[Theory]` methods; 011-P7's `dotnet test` run: 414 cases); `endpoints` maps 13 Sales routes (18 total), 0 unpoliced; `permissions` shows `accounts.*`/`contacts.*`/`deals.*`/`deals.discount.approve` all enforced. ~~Known gaps (plan 011)~~ — *closed by 011 (#45–#51):* guard violations and malformed cursors are 400 `ValidationProblemDetails` via `DomainValidationException` + `ApiExceptionHandler`; add-line is 422 on `won`/`lost`, stamps/enforces the frozen price-list version, and moves the deal's `xmin`; contact/deal read models carry `accountName`. Residual: `POST /api/deals/{id}/approve-discount` still answers a missing reason with an ad-hoc `{error}` 400 body, not `ValidationProblemDetails` (`DealEndpoints.cs:206`) |
...
| React console: shell, auth, Sales screens | ✅ built (Sales only) | `frontend/console` | measured 2026-09-24 (231c593, 010 P1–P8 done): 14 spec files under `frontend/`; token session + permission-gated nav (disabled-not-hidden); Accounts, Contacts and Deals grids + detail, deal lifecycle and discount approval; Development-only dev sign-in picker (`/api/dev/*`, AllowAnonymous, Development only). Orders/Comms/Assistant sections have no screens (no backend yet). ~~Known gaps (plan 011)~~ — *closed 2026-09-25:* selected rows carry `aria-current` (011-P6); account names come from the server read model (011-P5). `npm run test` 200 passed (011-P6) |
