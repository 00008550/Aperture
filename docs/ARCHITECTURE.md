...
| Sales: accounts, contacts, deals | ✅ built | `src/Modules/Sales` — 4 tables in the `sales` schema (`accounts`, `contacts`, `deals`, `deal_lines`); 13 order-agnostic routes on `accounts.*`/`contacts.*`/`deals.*` | measured 2026-09-24 (231c593): Sales 71 test methods, Api 80; `endpoints` maps 13 Sales routes (18 total), 0 unpoliced; `permissions` shows `accounts.*`/`contacts.*`/`deals.*`/`deals.discount.approve` all enforced. **Known gaps (plan 011):** domain guard violations (blank name, negative amount/credit/terms, discount outside 0–100, non-positive quantity) and malformed list cursors surface as **500** — the API has no exception handler; add-line accepts lines on `won`/`lost`/`quoted` deals and does not bump the deal's `xmin` |
...
| React console: shell, auth, Sales screens | ✅ built (Sales only) | `frontend/console` | measured 2026-09-24 (231c593, 010 P1–P8 done): 14 spec files under `frontend/`; token session + permission-gated nav (disabled-not-hidden); Accounts, Contacts and Deals grids + detail, deal lifecycle and discount approval; Development-only dev sign-in picker (`/api/dev/*`, AllowAnonymous, Development only). Orders/Comms/Assistant sections have no screens (no backend yet). **Known gaps (plan 011):** grids set `aria-selected` on plain `<tr>` (not exposed); account names resolve client-side from the first 50 accounts |
