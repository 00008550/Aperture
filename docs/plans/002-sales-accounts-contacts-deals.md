...
8. **Account reassignment re-stamps children (owned by P4).** Given account `acc` reassigned from agent `u1` to `u2` (and/or region `r1→r2`, team `t1→t2`) via `PATCH /api/accounts/{id}`; Then **all** its child rows — its contacts and its deals — have their denormalized inherited scope columns (`owner_user_id/team_id/region_id`; `tenant_id` and `account_id` are immutable) updated in the **same transaction** as the account edit; and a `Self(u1)` agent no longer sees `acc`, its contacts, or its deals while `Self(u2)` does, through **both** the EF path and the RLS/`ScopedConnection` grid. **Known gap on master until P4 lands:** as of P3's merge, `AccountService.UpdateAsync` re-stamps neither contacts nor deals, so a reassigned account leaves its contacts' scope columns stale (a contact stays visible under the old region/team grant and invisible under the new). P4 closes this.
...
### [x] P4 — Deals + deal lines: aggregate, creation, scoped grid (no transitions yet); account-reassignment re-stamps all children (edge 8)
**Touches:** `Sales/Domain/Deal.cs`, `Sales/Domain/DealLine.cs`, config + migration for `sales.deals` and `sales.deal_lines` (+ RLS on both), a new `Sales` deal application service, `Aperture.Api/Endpoints/DealEndpoints.cs` (create, get, add line, grid), tests; **and, to give edge 8 its home:** `Sales/Application/AccountService.cs` (the reassignment path — `UpdateAsync`), `Sales/Domain/Account.cs` (a re-stamp method that returns the new inherited scope tuple, replacing the "arrives with those aggregates (P3/P4)" comment with the concrete behaviour), and `Sales/Domain/Contact.cs` (a `Reinherit(account)` mutator so a contact's `owner_user_id/team_id/region_id` can be re-stamped from its reassigned parent — `tenant_id`/`account_id` stay immutable). Deal gets the equivalent re-stamp on the same path. This crosses no module boundary and stays within Sales; the child re-stamp is the natural completion of P2's deferred edge 8, and P4 is where the second child type (deals) finally exists, so both halves land together in one reviewable slice.
**Done when:** a deal is created in `new`, owned by one agent, scope columns inherited from its account; lines (price, quantity, price-list version) are added and saved with the deal; grid under `deals.read`, `xmin` on the deal. **And:** reassigning an account via `PATCH /api/accounts/{id}` (owner/region/team) re-stamps **every** child row — its contacts *and* its deals — inherited scope columns in the **same transaction** as the account edit, so no child is left visible under a stale grant; `tenant_id` and `account_id` are never mutated.
**Tests:** create/read/add-line; scope filtering EF + RLS on deals; tenant isolation. **Edge 8 (both halves), the named re-stamp test:** given `acc` owned by `u1` in region `r1`/team `t1` with a contact and a deal under it, when `acc` is reassigned to `u2`/`r2`/`t2`, then the contact's and the deal's `owner_user_id/region_id/team_id` all become `u2/r2/t2` in the same transaction; a `Self(u1)`/`Region(r1)`/`Team(t1)` agent then sees neither the account nor its contact nor its deal, and a `Self(u2)`/`Region(r2)`/`Team(t2)` agent sees all three — asserted on **both** the EF write-model read and the `ScopedConnection`/RLS grid (a child becomes visible under the NEW grant and not the old). Also assert the re-stamp and the account edit either both commit or both roll back (single unit of work), and that `account_id` is unchanged.
**Risk:** medium — the widest table and the aggregate boundary (deal owns lines), plus the cross-child re-stamp transaction (edge 8) that P2/P3 deliberately left to this portion.

### [x] P5 — Deal state machine: table-driven transitions + rules 1, 2, 4
