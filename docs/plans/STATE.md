...
| 009 | [Raw SQL scope safety: a correct Dapper path, and a gate](009-raw-sql-scope-safety.md) | in-progress | P3 | P4 |
...
| 009-P3 | `ScopedConnection`: the only door to raw SQL | build clean (warnings-as-errors); SharedKernel 64/64; `measure.sh gate` + `rawsql` green (0 production call sites); planted a second Dapper `PackageReference` — one-project test went red, then reverted; span/log observability and no-unscoped-overload asserted; no blocking review findings | [#24](https://github.com/00008550/Aperture/pull/24) — body in [`pr/009-P3.md`](pr/009-P3.md) |
